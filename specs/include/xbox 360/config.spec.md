# include/xbox 360/config.h Specification

## Source Context

- Source: `include/xbox 360/config.h`
- Related Headers: `config.h.in`, `configure.ac`
- Related Tests: `none`
- Related Dependencies: GitNexus `context "CONFIGURE_OPTION_TCP_LINGER" --file "include/xbox 360/config.h" --content --repo libsmb2` located `Macro:include/xbox 360/config.h:CONFIGURE_OPTION_TCP_LINGER` with no indexed incoming callers; GitNexus `impact "CONFIGURE_OPTION_TCP_LINGER" --include-tests --repo libsmb2` returned ambiguous because the same macro appears in multiple platform config headers; source search shows core files include `config.h` under `HAVE_CONFIG_H`, `lib/socket.c` uses `CONFIGURE_OPTION_TCP_LINGER`, and many C sources conditionally include standard headers through `HAVE_*` macros.
- Build/Compile Context: `configure.ac` generates `config.h` and defines Autoconf probes for TCP linger, headers, libraries, package metadata, and structure members; the Xbox 360 platform config is a checked-in generated configuration header with package version `4.0.0` while the current project context records top-level version `6.1.0`.

## Interface Summary

| Interface | Kind | Signature | Decision | Reason |
| --- | --- | --- | --- | --- |
| CONFIGURE_OPTION_TCP_LINGER | macro | #define CONFIGURE_OPTION_TCP_LINGER 1 | Include | Xbox 360 配置公开 TCP linger 构建开关，影响 socket 编译分支。 |
| HAVE_ARPA_INET_H | macro | /* #undef HAVE_ARPA_INET_H */ | Include | Xbox 360 配置显式声明 `<arpa/inet.h>` 不可用，影响网络地址头条件编译。 |
| HAVE_DLFCN_H | macro | /* #undef HAVE_DLFCN_H */ | Include | Xbox 360 配置显式声明 `<dlfcn.h>` 不可用，属于平台能力契约。 |
| HAVE_ERRNO_H | macro | #define HAVE_ERRNO_H 1 | Include | Xbox 360 配置声明 `<errno.h>` 可用，影响错误处理头条件编译。 |
| HAVE_FCNTL_H | macro | #define HAVE_FCNTL_H 1 | Include | Xbox 360 配置声明 `<fcntl.h>` 可用，影响文件控制头条件编译。 |
| HAVE_GSSAPI_GSSAPI_H | macro | /* #undef HAVE_GSSAPI_GSSAPI_H */ | Include | Xbox 360 配置显式声明 GSSAPI 头不可用，影响认证能力条件编译。 |
| HAVE_INTTYPES_H | macro | /* #undef HAVE_INTTYPES_H */ | Include | Xbox 360 配置显式声明 `<inttypes.h>` 不可用，影响整数格式头条件编译。 |
| HAVE_LIBKRB5 | macro | /* #undef HAVE_LIBKRB5 */ | Include | Xbox 360 配置显式声明 libkrb5 不可用，影响 Kerberos 认证条件编译。 |
| HAVE_LIBNSL | macro | /* #undef HAVE_LIBNSL */ | Include | Xbox 360 配置显式声明 `nsl` 库不可用，属于链接能力契约。 |
| HAVE_LIBSOCKET | macro | /* #undef HAVE_LIBSOCKET */ | Include | Xbox 360 配置显式声明 `socket` 库不可用，属于链接能力契约。 |
| HAVE_LINGER | macro | #define HAVE_LINGER 1 | Include | Xbox 360 配置声明系统提供 `struct linger`，影响本地兼容结构定义。 |
| HAVE_NETDB_H | macro | /* #undef HAVE_NETDB_H */ | Include | Xbox 360 配置显式声明 `<netdb.h>` 不可用，影响地址解析头条件编译。 |
| HAVE_NETINET_IN_H | macro | /* #undef HAVE_NETINET_IN_H */ | Include | Xbox 360 配置显式声明 `<netinet/in.h>` 不可用，影响 socket 类型条件编译。 |
| HAVE_NETINET_TCP_H | macro | /* #undef HAVE_NETINET_TCP_H */ | Include | Xbox 360 配置显式声明 `<netinet/tcp.h>` 不可用，影响 TCP 选项头条件编译。 |
| HAVE_POLL_H | macro | /* #undef HAVE_POLL_H */ | Include | Xbox 360 配置显式声明 `<poll.h>` 不可用，影响 poll 头选择。 |
| HAVE_SOCKADDR_LEN | macro | /* #undef HAVE_SOCKADDR_LEN */ | Include | Xbox 360 配置显式声明 sockaddr 不含 `sa_len` 成员，影响地址结构布局假设。 |
| HAVE_SOCKADDR_STORAGE | macro | /* #undef HAVE_SOCKADDR_STORAGE */ | Include | Xbox 360 配置显式声明 `sockaddr_storage` 不可用，影响通用 socket 地址存储假设。 |
| HAVE_STDINT_H | macro | #define HAVE_STDINT_H 1 | Include | Xbox 360 配置声明 `<stdint.h>` 可用，影响固定宽度整数头条件编译。 |
| HAVE_STDIO_H | macro | #define HAVE_STDIO_H 1 | Include | Xbox 360 配置声明 `<stdio.h>` 可用，影响标准 I/O 头条件编译。 |
| HAVE_STDLIB_H | macro | #define HAVE_STDLIB_H 1 | Include | Xbox 360 配置声明 `<stdlib.h>` 可用，影响分配和转换头条件编译。 |
| HAVE_STRINGS_H | macro | /* #undef HAVE_STRINGS_H 1 */ | Include | Xbox 360 配置显式声明 `<strings.h>` 不可用，影响 BSD 字符串头条件编译。 |
| HAVE_STRING_H | macro | #define HAVE_STRING_H 1 | Include | Xbox 360 配置声明 `<string.h>` 可用，影响字符串和内存操作头条件编译。 |
| HAVE_SYS_ERRNO_H | macro | /* #undef HAVE_SYS_ERRNO_H 1 */ | Include | Xbox 360 配置显式声明 `<sys/errno.h>` 不可用，影响系统 errno 头选择。 |
| HAVE_SYS_FCNTL_H | macro | /* #undef HAVE_SYS_FCNTL_H */ | Include | Xbox 360 配置显式声明 `<sys/fcntl.h>` 不可用，影响系统 fcntl 头选择。 |
| HAVE_SYS_IOCTL_H | macro | /* #undef HAVE_SYS_IOCTL_H */ | Include | Xbox 360 配置显式声明 `<sys/ioctl.h>` 不可用，影响 ioctl 头条件编译。 |
| HAVE_SYS_POLL_H | macro | /* #undef HAVE_SYS_POLL_H */ | Include | Xbox 360 配置显式声明 `<sys/poll.h>` 不可用，影响系统 poll 头选择。 |
| HAVE_SYS_SOCKET_H | macro | /* #undef HAVE_SYS_SOCKET_H */ | Include | Xbox 360 配置显式声明 `<sys/socket.h>` 不可用，影响 socket API 头条件编译。 |
| HAVE_SYS_STAT_H | macro | #define HAVE_SYS_STAT_H 1 | Include | Xbox 360 配置声明 `<sys/stat.h>` 可用，影响文件状态头条件编译。 |
| HAVE_SYS_TIME_H | macro | /* #undef HAVE_SYS_TIME_H 1 */ | Include | Xbox 360 配置显式声明 `<sys/time.h>` 不可用，影响时间头条件编译。 |
| HAVE_SYS_TYPES_H | macro | #define HAVE_SYS_TYPES_H 1 | Include | Xbox 360 配置声明 `<sys/types.h>` 可用，影响系统类型头条件编译。 |
| HAVE_SYS_UIO_H | macro | /* #undef HAVE_SYS_UIO_H */ | Include | Xbox 360 配置显式声明 `<sys/uio.h>` 不可用，影响 scatter/gather I/O 头条件编译。 |
| HAVE_SYS_UNISTD_H | macro | /* #undef HAVE_SYS_UNISTD_H */ | Include | Xbox 360 配置显式声明 `<sys/unistd.h>` 不可用，影响系统 unistd 头选择。 |
| HAVE_SYS__IOVEC_H | macro | /* #undef HAVE_SYS__IOVEC_H */ | Include | Xbox 360 配置显式声明 `<sys/_iovec.h>` 不可用，影响 iovec 头选择。 |
| HAVE_TIME_H | macro | #define HAVE_TIME_H 1 | Include | Xbox 360 配置声明 `<time.h>` 可用，影响时间头条件编译。 |
| HAVE_UNISTD_H | macro | /* #undef HAVE_UNISTD_H */ | Include | Xbox 360 配置显式声明 `<unistd.h>` 不可用，影响 POSIX API 头条件编译。 |
| LT_OBJDIR | macro | #define LT_OBJDIR ".libs/" | Include | Xbox 360 配置公开 libtool 对象目录字符串，属于生成配置头元数据。 |
| PACKAGE | macro | #define PACKAGE "libsmb2" | Include | Xbox 360 配置公开包短名，属于包元数据契约。 |
| PACKAGE_BUGREPORT | macro | #define PACKAGE_BUGREPORT "ronniesahlberg@gmail.com" | Include | Xbox 360 配置公开错误报告地址，属于包元数据契约。 |
| PACKAGE_NAME | macro | #define PACKAGE_NAME "libsmb2" | Include | Xbox 360 配置公开包全名，属于包元数据契约。 |
| PACKAGE_STRING | macro | #define PACKAGE_STRING "libsmb2 4.0.0" | Include | Xbox 360 配置公开包名和版本组合，属于包元数据契约。 |
| PACKAGE_TARNAME | macro | #define PACKAGE_TARNAME "libsmb2" | Include | Xbox 360 配置公开发行包名，属于包元数据契约。 |
| PACKAGE_URL | macro | #define PACKAGE_URL "" | Include | Xbox 360 配置公开包主页为空字符串，属于包元数据契约。 |
| PACKAGE_VERSION | macro | #define PACKAGE_VERSION "4.0.0" | Include | Xbox 360 配置公开包版本，属于包元数据契约。 |
| STDC_HEADERS | macro | #define STDC_HEADERS 1 | Include | Xbox 360 配置声明 C90 标准头集合可用，影响标准头条件编译。 |
| VERSION | macro | #define VERSION "4.0.0" | Include | Xbox 360 配置公开版本号别名，属于包元数据契约。 |

## Data Model Summary

| Type/Macro | Kind | Definition | Notes |
| --- | --- | --- | --- |
| CONFIGURE_OPTION_TCP_LINGER | macro | include/xbox 360/config.h:5 | 取值为 `1`，表示 Xbox 360 配置允许 TCP socket linger；`lib/socket.c` 只在该值为 `0` 时编译设置 `SO_REUSEADDR` 和零秒 `SO_LINGER` 的分支。 |
| HAVE_* | macro group | include/xbox 360/config.h:8 | 平台能力宏以定义或 `#undef` 注释形式记录 Xbox 360 可用头文件、库和结构能力；具体宏详见 Interface Summary。 |
| PACKAGE | macro | include/xbox 360/config.h:113 | 包元数据宏声明 Xbox 360 配置头中的包名、版本、URL 和联系信息；同类宏详见 Interface Summary。 |
| LT_OBJDIR | macro | include/xbox 360/config.h:110 | 取值为 `".libs/"`，保留 Autoconf/libtool 对象目录元数据。 |
| STDC_HEADERS | macro | include/xbox 360/config.h:136 | 取值为 `1`，保留 Autoconf 标准头兼容信号。 |

## ADDED Requirements

### Requirement: CONFIGURE_OPTION_TCP_LINGER controls Xbox 360 socket linger policy
系统 MUST 在 Xbox 360 配置头中将 `CONFIGURE_OPTION_TCP_LINGER` 暴露为值 `1`，使包含 `config.h` 的 socket 源码看到 Xbox 360 配置允许 TCP linger 的编译时策略。

#### Scenario: Xbox 360 配置保留默认 linger 行为
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_ARPA_INET_H`，以声明 `<arpa/inet.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_DLFCN_H`，以声明 `<dlfcn.h>` 在该配置中不可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_ERRNO_H` 为 `1`，表示 `<errno.h>` 对错误处理条件编译可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_FCNTL_H` 为 `1`，表示 `<fcntl.h>` 对条件编译使用方可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_GSSAPI_GSSAPI_H`，以声明 `<gssapi/gssapi.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_INTTYPES_H`，以声明 `<inttypes.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_LIBKRB5`，以声明 libkrb5 支持在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_LIBNSL`，以声明 `nsl` 链接库在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_LIBSOCKET`，以声明单独的 `socket` 链接库在该配置中不可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_LINGER` 为 `1`，表示平台提供 `struct linger`。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_NETDB_H`，以声明 `<netdb.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_NETINET_IN_H`，以声明 `<netinet/in.h>` 在该配置中不可用。

#### Scenario: netinet in header remains disabled
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_NETINET_TCP_H`，以声明 `<netinet/tcp.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_POLL_H`，以声明 `<poll.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SOCKADDR_LEN`，以声明 sockaddr 结构没有 `sa_len` 成员。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SOCKADDR_STORAGE`，以声明该配置不公开 `sockaddr_storage` 可用性。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_STDINT_H` 为 `1`，表示 `<stdint.h>` 对固定宽度整数条件编译可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_STDIO_H` 为 `1`，表示 `<stdio.h>` 对条件编译使用方可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_STDLIB_H` 为 `1`，表示 `<stdlib.h>` 对条件编译使用方可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_STRINGS_H`，以声明 `<strings.h>` 在该配置中不可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_STRING_H` 为 `1`，表示 `<string.h>` 对条件编译使用方可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_ERRNO_H`，以声明 `<sys/errno.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_FCNTL_H`，以声明 `<sys/fcntl.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_IOCTL_H`，以声明 `<sys/ioctl.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_POLL_H`，以声明 `<sys/poll.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_SOCKET_H`，以声明 `<sys/socket.h>` 在该配置中不可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_SYS_STAT_H` 为 `1`，表示 `<sys/stat.h>` 对文件状态条件编译可用。

#### Scenario: sys stat header capability is visible
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_TIME_H`，以声明 `<sys/time.h>` 在该配置中不可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_SYS_TYPES_H` 为 `1`，表示 `<sys/types.h>` 对条件编译使用方可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_UIO_H`，以声明 `<sys/uio.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS_UNISTD_H`，以声明 `<sys/unistd.h>` 在该配置中不可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_SYS__IOVEC_H`，以声明 `<sys/_iovec.h>` 在该配置中不可用。
...
系统 MUST 在 Xbox 360 配置头中定义 `HAVE_TIME_H` 为 `1`，表示 `<time.h>` 对时间源码条件编译可用。
...
系统 MUST NOT 在 Xbox 360 配置头中定义 `HAVE_UNISTD_H`，以声明 `<unistd.h>` 在该配置中不可用。

#### Scenario: unistd header remains disabled
...
系统 MUST 在 Xbox 360 配置头中定义 `LT_OBJDIR` 为字符串 `".libs/"`，保留生成配置头的 libtool 对象目录元数据。
...
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE` 为字符串 `"libsmb2"`，表示包短名。
...
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE_BUGREPORT` 为字符串 `"ronniesahlberg@gmail.com"`，表示问题报告地址。
...
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE_NAME` 为字符串 `"libsmb2"`，表示包全名。
...
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE_STRING` 为字符串 `"libsmb2 4.0.0"`，表示包名和版本组合。
...
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE_TARNAME` 为字符串 `"libsmb2"`，表示发行包名。
...
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE_URL` 为空字符串，表示该配置未声明主页 URL。
...
### Requirement: PACKAGE_VERSION exposes Xbox 360 package version
系统 MUST 在 Xbox 360 配置头中定义 `PACKAGE_VERSION` 为字符串 `"4.0.0"`，表示该配置头记录的包版本。
...
系统 MUST 在 Xbox 360 配置头中定义 `STDC_HEADERS` 为 `1`，表示 C90 标准头集合可用。
...
### Requirement: VERSION exposes Xbox 360 version string
系统 MUST 在 Xbox 360 配置头中定义 `VERSION` 为字符串 `"4.0.0"`，表示该配置头记录的版本号。
...
| --- | --- | --- | --- |
| Q-001 | Xbox 360 配置头中的 `PACKAGE_VERSION`/`VERSION` 为 `4.0.0`，而顶层 CMake 项目版本和 `configure.ac` 当前项目版本为 `6.1.0`；该差异是否为有意保留的 Xbox 360 固定配置版本？ | PACKAGE_VERSION, VERSION, PACKAGE_STRING | 源码证据确认存在版本差异，但未发现说明该差异的测试或文档。 |
| Q-002 | GitNexus `impact "CONFIGURE_OPTION_TCP_LINGER" --include-tests --repo libsmb2` 因多个平台 `config.h` 中同名宏而返回 ambiguous；Xbox 360 单宏的完整上游影响是否仅限源码搜索发现的 `lib/socket.c` 分支仍待确认。 | CONFIGURE_OPTION_TCP_LINGER | GitNexus 已定位当前文件宏 context，但 impact 无法按文件消歧。 |
