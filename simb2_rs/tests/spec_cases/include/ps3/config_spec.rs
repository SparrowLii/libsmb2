use libsmb2_sys::include::config::PS3_CONFIG;
...
// Spec: CONFIGURE_OPTION_TCP_LINGER controls PS3 socket linger policy#PS3 配置保留默认 linger 行为
...
#[test]
fn test_config_ps3_linger() {
    assert_eq!(PS3_CONFIG.configure_option_tcp_linger, Some(1));
}
...
#[test]
fn test_config_arpa_inet_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_arpa_inet_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_dlfcn_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_dlfcn_h, None);
}
...
#[test]
fn test_config_errno_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_errno_h, Some(1));
}
...
#[test]
fn test_config_fcntl_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_fcntl_h, Some(1));
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_gssapi_header_capability_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_gssapi_gssapi_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_inttypes_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_inttypes_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_kerberos_library_capability_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_libkrb5, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_nsl_link_capability_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_libnsl, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_socket_link_capability_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_libsocket, None);
}
...
#[test]
fn test_config_compatible_linger_structure_is_not_selected() {
    assert_eq!(PS3_CONFIG.have_linger, Some(1));
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_netdb_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_netdb_h, None);
}

// Trace: `include/ps3/config.h:HAVE_NETINET_IN_H`, `lib/socket.c:38`, `lib/md5.h:30`
// Spec: HAVE_NETINET_IN_H declares netinet in header availability#netinet in header capability is visible
// - **GIVEN** 源码以配置宏判断 IP socket 类型头能力
// - **WHEN** 读取 `HAVE_NETINET_IN_H`
// - **THEN** 该宏 SHALL 以定义值 `1` 表示 `<netinet/in.h>` 可用
#[test]
fn test_config_netinet_in_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_netinet_in_h, Some(1));
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_netinet_tcp_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_netinet_tcp_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_poll_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_poll_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sockaddr_length_member_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sockaddr_len, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sockaddr_storage_capability_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sockaddr_storage, None);
}
...
#[test]
fn test_config_stdint_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_stdint_h, Some(1));
}
...
#[test]
fn test_config_stdio_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_stdio_h, Some(1));
}
...
#[test]
fn test_config_stdlib_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_stdlib_h, Some(1));
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_strings_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_strings_h, None);
}
...
#[test]
fn test_config_string_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_string_h, Some(1));
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_errno_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_errno_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_fcntl_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_fcntl_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_ioctl_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_ioctl_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_poll_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_poll_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_socket_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_socket_h, None);
}

// Trace: `include/ps3/config.h:HAVE_SYS_STAT_H`
// Spec: HAVE_SYS_STAT_H declares sys stat header absence#sys stat header remains disabled
// - **GIVEN** 源码以配置宏判断文件状态头能力
// - **WHEN** 读取 PS3 配置头中的 `HAVE_SYS_STAT_H`
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_stat_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_stat_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_time_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_time_h, None);
}
...
#[test]
fn test_config_sys_types_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_sys_types_h, Some(1));
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_uio_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_uio_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_unistd_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_unistd_h, None);
}
...
// - **THEN** 该宏 SHALL 保持未定义状态
#[test]
fn test_config_sys_iovec_header_remains_disabled() {
    assert_eq!(PS3_CONFIG.have_sys_iovec_h, None);
}
...
#[test]
fn test_config_time_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_time_h, Some(1));
}

// Trace: `include/ps3/config.h:HAVE_UNISTD_H`, `lib/socket.c:78`, `configure.ac:HAVE_UNISTD_H`
// Spec: HAVE_UNISTD_H declares unistd header availability#unistd header capability is visible
// - **GIVEN** 源码以配置宏判断 POSIX 头能力
// - **WHEN** 读取 `HAVE_UNISTD_H`
// - **THEN** 该宏 SHALL 以定义值 `1` 表示 `<unistd.h>` 可用
#[test]
fn test_config_unistd_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.have_unistd_h, Some(1));
}
...
#[test]
fn test_config_libtool_object_directory_metadata_is_readable() {
    assert_eq!(PS3_CONFIG.lt_objdir, ".libs/");
}
...
#[test]
fn test_config_package_short_name_is_readable() {
    assert_eq!(PS3_CONFIG.package, "libsmb2");
}
...
#[test]
fn test_config_bug_report_contact_is_readable() {
    assert_eq!(PS3_CONFIG.package_bugreport, "ronniesahlberg@gmail.com");
}
...
// - **THEN** 该宏 SHALL 展开为字符串 `"libsmb2"`
#[test]
fn test_config_package_full_name_is_readable() {
    assert_eq!(PS3_CONFIG.package_name, "libsmb2");
}
...
#[test]
fn test_config_package_string_is_readable() {
    assert_eq!(PS3_CONFIG.package_string, "libsmb2 4.0.0");
}
...
// - **THEN** 该宏 SHALL 展开为字符串 `"libsmb2"`
#[test]
fn test_config_distribution_tar_name_is_readable() {
    assert_eq!(PS3_CONFIG.package_tarname, "libsmb2");
}
...
#[test]
fn test_config_package_url_metadata_is_readable() {
    assert_eq!(PS3_CONFIG.package_url, "");
}

// Trace: `include/ps3/config.h:PACKAGE_VERSION`
// Spec: PACKAGE_VERSION exposes PS3 package version#package version is readable
// - **GIVEN** 源码或诊断工具读取包元数据
// - **WHEN** 读取 `PACKAGE_VERSION`
// - **THEN** 该宏 SHALL 展开为字符串 `"4.0.0"`
#[test]
fn test_config_package_version_is_readable() {
    assert_eq!(PS3_CONFIG.package_version, "4.0.0");
}
...
#[test]
fn test_config_standard_header_capability_is_visible() {
    assert_eq!(PS3_CONFIG.stdc_headers, Some(1));
}

// Trace: `include/ps3/config.h:VERSION`
// Spec: VERSION exposes PS3 version string#version string is readable
// - **GIVEN** 源码或诊断工具读取版本元数据
// - **WHEN** 读取 `VERSION`
// - **THEN** 该宏 SHALL 展开为字符串 `"4.0.0"`
#[test]
fn test_config_version_string_is_readable() {
    assert_eq!(PS3_CONFIG.version, "4.0.0");
}
