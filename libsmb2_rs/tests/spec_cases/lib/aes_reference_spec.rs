...
// Trace: `lib/aes_reference.c:AES128_CBC_encrypt_buffer_reference`, `lib/aes_reference.h:AES128_CBC_encrypt_buffer_reference`
// Spec: AES128_CBC_encrypt_buffer_reference encrypts CBC buffers with zero padding#encrypt partial final CBC block
// - **GIVEN** `CBC` 预处理条件为真，`length % 16 != 0`，且输入缓冲区在尾部剩余不足 16 字节
// - **WHEN** 调用 `AES128_CBC_encrypt_buffer_reference(output, input, length, key, iv)`
// - **THEN** 函数复制尾部剩余字节到输出尾块，将剩余位置填充为零，并对该 16 字节尾块执行 AES-128 加密
#[test]
fn test_aes_reference_encrypt_partial_final_cbc_block() {
    let key = AesBlock([0x00; 16]);
    let iv = AesBlock([0x00; 16]);
    let plaintext = [0x00; 5];
...
// Spec: AES128_CBC_decrypt_buffer_reference decrypts CBC buffers with zero-filled tail handling#decrypt full CBC blocks
// - **GIVEN** `CBC` 预处理条件为真，`length` 为 16 的整数倍，`input` 与 `output` 覆盖 `length` 字节，`key` 指向 16 字节 AES-128 key，且 `iv` 指向 16 字节初始向量
...
#[test]
fn test_aes_reference_decrypt_full_cbc_blocks() {
    let key = AesBlock([0x00; 16]);
    let iv = AesBlock([0x00; 16]);
    let plaintext = [0x00; 16];
    let ciphertext = reference_cbc_encrypt(&plaintext, key, iv);

    let decrypted = reference_cbc_decrypt(&ciphertext, key, iv);

    assert_eq!(decrypted, plaintext);
}

// Trace: `lib/aes_reference.c:AES128_CBC_decrypt_buffer_reference`, `lib/aes_reference.h:AES128_CBC_decrypt_buffer_reference`
// Spec: AES128_CBC_decrypt_buffer_reference decrypts CBC buffers with zero-filled tail handling#decrypt partial final CBC block
// - **GIVEN** `CBC` 预处理条件为真，`length % 16 != 0`，且输入缓冲区在尾部剩余不足 16 字节
// - **WHEN** 调用 `AES128_CBC_decrypt_buffer_reference(output, input, length, key, iv)`
// - **THEN** 函数复制尾部剩余字节到输出尾块，将剩余位置填充为零，并对该 16 字节尾块执行 AES-128 inverse cipher
#[test]
fn test_aes_reference_decrypt_partial_final_cbc_block() {
    let key = AesBlock([0x00; 16]);
    let iv = AesBlock([0x00; 16]);
    let ciphertext = [0x66, 0xe9, 0x4b, 0xd4, 0xef];
    let padded_ciphertext = AesBlock([
        0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00,
    ]);

    let decrypted = reference_cbc_decrypt(&ciphertext, key, iv);

    assert_eq!(decrypted, reference_decrypt_block(padded_ciphertext, key).0);
}
